...
The idea behind inequality aversion is that people may have a dislike of having less than other people and a dislike of having more than other people.
...
@charness2002 developed a more general utility function that captures the possible forms of distributional preference. An agent's attitude toward others depends on their relative position. The utility function is:
...
A dictator has less than the other player. Their $\sigma=−1/2$ (they have less than the other player, so $\sigma$ is the relevant parameter). They must decide between the allocations (1, 0) and (5, 1).

They calculate the utility of each allocation.

```{=tex}
\begin{align*}
U(1,0)&=\sigma\times 1+(1−\sigma)\times 0 \\[6pt]
&=−1/2×1+(1+1/2)\times 0 \\[6pt]
&=−1/2 \\[12pt]
U(5,1)&=\sigma\times 5+(1−\sigma)\times 1 \\[6pt]
&=−1/2×5+(1+1/2)\times 1 \\[6pt]
&=−1
\end{align*}
```

They prefer to allocate (1,0), even though it is worse for them, because it is also worse for the other player.
...
If $1\geq\sigma\geq0$, the agent is altruistic. A higher payoff to the other player increases the agent's utility

If $1\geq\rho\geq0\geq\sigma$, the agent is inequality averse. If the other player has more, the agent's utility decreases with further gains for the other player. If the other player has less, the agent's utility increases with further gains for either agent.

If $0\geq\rho\geq\sigma$, the agent is status-seeking. They gain more utility by having more than the other player. Their utility goes up when either they get more or the other player gets less.
