...
We can see evidence for reciprocity in how people respond to the intentions of others.
...
```{mermaid}
%%| fig-width: 4
%%| label: fig-ultimatum-game-scenario-1
%%| fig-cap: Scenario 1 of the ultimatum game
%%| mermaid-format: png

graph LR
    A(Proposer) ---B[Offer 2] --> C(Responder)
    A(Proposer) ---D[Offer 5] --> E(Responder)
    C ---F[Accept] --> H["(8, 2)"]
    C ---G[Reject] --> I["(0, 0)"]
    E ---J[Accept] --> L["(5, 5)"]
    E ---K[Reject] --> M["(0, 0)"]
...
```{mermaid}
%%| fig-width: 4
%%| label: fig-dictator-game-scenario-2
%%| fig-cap: Scenario 2 of the ultimatum game
%%| mermaid-format: png
...
Rejection in this case cannot be explained by distributional concerns. An offer of (\$8, \$2) leads to the same distribution in both scenarios.
...
The trust game provides another potential example of reciprocity.
...
Before the investment is received by the receiver, it is multiplied by some factor $k$. Therefore, the receiver receives $kx$.
...
One possible explanations for this behaviour is that the receiver feels that they should reciprocate the sender's investment. They are responding to the sender's intentions. The sender trusts that due to reciprocation some of their investment will be repaid.
...
In this game, each of N participants is given an initial endowment.
...
```{mermaid}
%%| fig-width: 5
%%| label: fig-public-goods-game-4
%%| fig-cap: The public goods game
%%| mermaid-format: png

graph TD
    classDef default fill:#FFF
    A[Player A] ---F[Contribute x] --> K[Pot]
    B[Player B] ---G[Contribute y] --> K[Pot]
    C[Player C] ---H[Contribute z] --> K[Pot]
    D[Player D] ---I[Contribute w] --> K[Pot]
    E[Player E] ---J[Contribute v] --> K[Pot]

    K ---L{Multiplied by k} --> M[Multiplied Pot]

    M ---N[Receive share]--> S[Player A]
    M ---O[Receive share]--> T[Player B]
    M ---P[Receive share]--> U[Player C]
    M ---Q[Receive share]--> V[Player D]
    M ---R[Receive share]--> W[Player E]
...
In a meta-analysis, @zelmer2003 found an average contribution of 38% of the endowment. The amnount contributed increased with the marginal per capita return; that is the higher $k/N$.
