...
&=0+\frac{3}{4}\times \frac{2}{3}\times 12 \\
&=6
\end{align*}
...
At $t=0$ the difference in discount between the \$4 at $t=1$ and \$7 at $t=2$ is $\delta$. Both are discounted by $\beta$ as neither is immediately available. However, at $t=0$ the difference in discount between the \$4 and \$7 is $\beta\delta$. The \$4 is no longer subject to the immediate discount of $\beta$, making it relatively more attractive.
:::
...
a\) Alfred and Blake are exponential discounters. They can choose between a small reward now or a larger reward later. Alfred discounts the future heavily (low $\delta$). Blake does not ($\delta$ close to one).

How might Alfred and Blake’s choices be affected by their discount factor?
