...
Consider a sophisticated present-biased agent with $\beta=0.5$ and $\delta=1$ choosing between three movies:
...
To determine their action, they solve by backward induction. First, what will their decision be next week?
...
```{=tex}
\begin{align*}
U_0(\text{exercise})&=u(x_0)+\beta\delta u(x_1) \\[6pt]
&=u(\text{exercise})+\beta\delta u(\text{healthy}) \\[6pt]
&=0+\frac{1}{2}\times \frac{2}{3}\times 12 \\[6pt]
&=4 \\[6pt]
\\[6pt]
U_0(\text{television})&=u(x_0)+\beta\delta u(x_1) \\[6pt]
&=u(\text{television})+\beta\delta u(\text{unhealthy}) \\[6pt]
...
```{=tex}
\begin{align*}
U_0(\text{exercise+massage})&=u(x_0)+\beta\delta u(x_1) \\[6pt]
&=u(\text{exercise})+u(\text{massage})\\[6pt]
