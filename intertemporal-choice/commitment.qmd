...
Recall what the sophisticated present-biased agent with $\beta=0.5$ and $\delta=1$ did when faced with a choice of (0, \$6; 1, \$10; 2, \$16). First they realised that next week they would take the \$10 straight away.
...
But from the perspective of Odysseus today, with both the benefits of the siren and living in the future discounted, Odysseus would prefer to live. As a result, he decides to commit himself to that course of action by instructing his crew to tie him to the mast (plus leaving his ears unplugged so that he also gets some benefits from the siren song).
...
At $t=1$ spending now has the highest discounted utility. After saving for the first period, you spend despite initially wanting to save.
...
If the agent had spent at $t=0$, you have no choice but to spend at $t=1$.
...
For payment of an administrative fee equivalent to 1 unit of utility and an initial deposit (the agent's savings in $t=0$), you reserve the jacket, preventing you from spending that money in period 2. The new set of options is:
...
Note that Lay-by is strictly worse than Save as the agent must pay the administrative fee. But the sophisticated quasi-hyperbolic discounter chooses it as the only feasible way to get their jacket. Without Lay-by they know they will Start spending at $t=1$ and end up with lower utility from the perspective of their $t=0$ self.
