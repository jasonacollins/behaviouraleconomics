...
Here are two formal definitions (the first more intuitive):
...
2. If $x$, $y$ and $z$ are lotteries with $x\succcurlyeq y\succcurlyeq z$, the continuity axiom requires that there exists a probability $p$ such that $y$ is equally as good as a mix of $x$ and $z$. That is, there exists $p$ such that:
...
Note: If you read the literature about expected utility, you may come across an axiom called the Archimedean property. Only one of continuity (as I define it) or the Archimedean property need be assumed. I will not cover the Archimedean property in this book.
...
Consider an agent with lexiographic preferences is offered the following combinations of $x$ and $y$.
