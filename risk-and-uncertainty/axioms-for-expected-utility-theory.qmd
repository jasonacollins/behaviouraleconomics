...
Importantly, preferences under these assumptions are cardinal.
...
ggplot()+
  ...
  labs(x = "", y = "") +
  coord_fixed(ratio = 1, xlim = c(0,20), ylim = c(0,20), expand = TRUE, clip = "on")
```

Note: If you read the literature about expected utility, you may come across an axiom called the Archimedean property. Only one of continuity or the Archimedean property need be assumed. I will not cover the Archimedean property in these notes.
...
ggplot()+
  ...
  geom_circle(aes(x0 = 1, y0 = 1, r = 0.05)) +
  geom_vline(xintercept = 0, size=0.25) + 
  geom_hline(yintercept = 0, size=0.25) +
  labs(x = "x", y = "y") +
  coord_fixed(ratio = 1, xlim = c(0,2.2), ylim = c(0,2.2), expand = TRUE, clip = "on")
```
