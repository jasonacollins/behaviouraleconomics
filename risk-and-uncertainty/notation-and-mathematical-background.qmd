...
A plot of the function $f(x)=x^{0.5}$ is shown in @fig-square-root.
...
A plot of the exponential function is shown in @fig-logarithmic-function.
...
You can see from this that for any value of $x$ greater than zero, the derivative of $x^{0.5}$ is greater than zero, signifying that the function $f(x)=x^{0.5}$ is increasing and has positive slope for $x>0$. The function is not defined for $x<0$. This is shown in @fig-square-root.
...
This derivative is positive for all values of $x$, signifying that the function $f(x)=e^x$ is increasing and has positive slope. You can see this in @fig-exponential-function.
...
The second derivative of a function is the derivative of the derivative. It is written as $\frac{d^2}{dx^2}f(x)$ or $\frac{d^2 y}{dx^2}$ or $f''(x)$.
...
You will not be asked to differentiate any functions when working through these notes. However, understanding what differentiation is and what it is showing will help you understand the intuition behind the concepts that I discuss. We will use the functions $f(x)=\ln(x)$ and $f(x)=x^{0.5}$ in future sections.
