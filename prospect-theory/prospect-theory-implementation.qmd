...
-   Coding: People perceive outcomes as gains or losses relative to some neutral reference position. The reference point may be the status quo or some other expectation of the agent.
...
-   Segregation: Riskless components are segregated from risky components. For example (300, 0.80; 200, 0.20) corresponds to a sure gain of 200 and the risky gamble (100, 0.80).

-   Cancellation: Components that are shared by two prospects are ignored.

## Evaluation

In the evaluation phase, the prospects are evaluated and the option with the highest value is chosen.
...
$V(X)$ is the expected value of the outcomes from gamble $X$.
